...
export class NetworkLoadBalancer extends BaseLoadBalancer implements INetworkLoadBalancer {
  ...
  constructor(scope: Construct, id: string, props: NetworkLoadBalancerProps) {
    ...
    if (minimumCapacityUnit && !Token.isUnresolved(minimumCapacityUnit)) {
      ...
      if (!Number.isInteger(minimumCapacityUnit) || capacityUnitPerAz < 2750 || capacityUnitPerAz > 90000) {
        throw new ValidationError(`'minimumCapacityUnit' must be a positive value between 2750 and 90000 per AZ for Network Load Balancer, got ${capacityUnitPerAz} LCU per AZ.`, this);
      }
    }
